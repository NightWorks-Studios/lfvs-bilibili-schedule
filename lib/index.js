...
var BilibiliScheduleService = class extends Service {
  constructor(ctx, config) {
    ...
    Promise.resolve().then(() => this.start().catch((e) => {
      this.ctx.emit("lfvs/log", "bilibili-schedule", "error", `启动失败: ${e.message}`);
    }));
  }
  config;
  static {
    ...
    try {
      ...
      for (const video of videosToUpdate) {
        ...
        try {
          ...
          totalFailure++;
          this.ctx.emit("lfvs/log", "bilibili-schedule", "error", `updateVideos 异常: ${error.message}`);
        }
        if (!this.abortController.signal.aborted) {
          try {
